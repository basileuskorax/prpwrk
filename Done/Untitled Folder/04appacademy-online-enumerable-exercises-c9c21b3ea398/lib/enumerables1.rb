...
def array_sum_with_index(arr)
  sum = 0
  arr.each_with_index{|e, i| sum += (e * i)}
  sum
end
...
def price_is_right(bids, actual_retail_price)
  not_enough = bids.reject{|bid| bid > actual_retail_price}
  not_enough.max
end
...
def ordered_vowel_word?(word)
  ...
  word_vowels = word.chars.select {|char| vowels.include?(char)}
...
def products_except_me(numbers)
  numbers.map.with_index {|e, i| array_product(numbers[0...i] + numbers[i+1..-1])}
end
